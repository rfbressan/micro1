---
title: "Relações de demanda entre bens"
...
# O caso de dois bens{.greenbackground}
...
![Efeito-cruzado de variação nos preços: complementares brutos. Fonte: Nicholson, Snyder (2016).](img/grosscomplements.JPG){#fig-fig1}


## Introdução

-   Como efeito de uma queda em $p_y$, percebemos na @fig-fig1 um aumento na
...
-   Na @fig-fig1, as curvas de indiferença representadas praticamente assumem um formato
...
-   Como $\partial x/\partial p_y<0$, dizemos que $x$ e $y$ são
    [complementares brutos]{style="color: red"}.


## Introdução

![Efeito-cruzado de variação nos preços: substitutos brutos. Fonte: Nicholson, Snyder (2016).](img/grosssubstitutes.JPG){#fig-fig2}

## Introdução

-   Na @fig-fig2, a situação é revertida. Agora $\partial x/\partial p_y > 0$.
...
-   Neste caso, $x$ e $p_y$ movem na mesma direção - [substitutos brutos]{style="color: red"}.
...
-   Podemos, ainda, escrever a @eq-eq1 em termos de
    elasticidades: 
$$\begin{aligned}
...
e_{x,p_y} &=& e_{x^c,p_y} - s_y e_{x,I}.
\end{aligned}$${#eq-eq3}
...
-   Para o caso de dois bens, os termos do lado direito das @eq-eq1 e
    @eq-eq3 possuem sinais algébricos distintos.
...
Suponha um consumidor cujas relações de preferência sejam
...
# Bens substitutos e complementares{.greenbackground}
...
-   Neste caso, podemos generalizar a equação de Slutsky para quaisquer
    ...
    $$\frac{\partial x_i(p_1, \dots, p_n, I)}{\partial p_j} = \left.\frac{\partial x_i}{\partial p_j}\right|_{U=k} - x_j\frac{\partial x_i}{\partial I}.$${#eq-eq4}
...
-   As @eq-eq4 e @eq-eq5 nos dizem que variações no preço de um bem $j$ qualquer induz efeitos renda e
    substituição que podem alterar a quantidade demandada de qualquer
    outro bem.

-   Dois bens são ditos [substitutos]{style="color: red"} se um bem,
...
-   Dois bens são ditos [complementares]{style="color: red"} se "vão
...
:::{.callout-tip title="Substitutos e complementares brutos"}
- Dois bens $x_i$ e $x_j$ são [substitutos brutos]{style="color: red"} se:
$$\frac{\partial x_i}{\partial p_j} > 0.$${#eq-eq6}

- Dois bens $x_i$ e $x_j$ são [complementares brutos]{style="color: red"} se:
$$\frac{\partial x_i}{\partial p_j} < 0.$${#eq-eq7}
:::
...
:::{.callout-tip title="Substitutos e complementares líquidos"}
- Os bens $x_i$ e $x_j$ são ditos [substitutos líquidos]{style="color: red"} se:
$$\left.\frac{\partial x_i}{\partial p_j}\right|_{U=k} > 0.$${#eq-eq8}

- Os bens $x_i$ e $x_j$ são ditos [complementares líquidos]{style="color: red"} se:
$$\left.\frac{\partial x_i}{\partial p_j}\right|_{U=k} < 0.$${#eq-eq9}
:::
...
-   A diferença entre as definições Marshallianas e Hicksianas para bens
    substitutos e complementares pode ser percebida nas @fig-fig1 e @fig-fig2.
...
-   Na @fig-fig1, o efeito renda negativo domina o efeito substituição positivo e,
    portanto, $\partial x/\partial p_y<0$ e os bens são denominados
    "complementares Marshallianos".

-   Já na @fig-fig2, o efeito substituição positivo domina o efeito renda negativo,
...
# Substituibilidade com vários bens{.greenbackground}
...
-   Este resultado é resumido no que ficou conhecido como [segunda lei da demanda de Hicks]{style="color: red"}.
...
-   Para mostrar este resultado, tomamos a função de demanda compensada
    ...
    Euler, temos:
    $$p_1 \frac{\partial x_i^c}{\partial p_1} + p_2 \frac{\partial x_i^c}{\partial p_2} + \dots + p_n \frac{\partial x_i^c}{\partial p_n} = 0.$${#eq-eq10}


## Substituibilidade com vários bens

-   A @eq-eq10 pode ser escrita em termos de elasticidades:
    $$e_{i1}^c + e_{i2}^c + \dots + e_{in}^c = 0.$${#eq-eq11}

-   Mas sabemos que $e_{ii}^c \leq 0$ devido à negatividade do efeito
    substituição com relação ao próprio bem - [primeira lei da demanda de Hicks]{style="color: red"}.[^1]

-   Portanto, deve ser o caso que:
    $$\sum_{j\neq i}e_{ij}^c \geq 0.$${#eq-eq12}
